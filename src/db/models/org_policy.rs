...
impl OrgPolicy {
    ...
    /// Requires both the server-wide option and this organization's policy.
...
/// Active auto-confirm policy memberships. No role or status is exempt; each check below decides which
/// statuses matter for its operation.
...
impl AutoConfirmRequirement {
    /// Loads every role and status in organizations with this policy; empty when the server option is off.
        ...
        Self(
            conn.run(move |conn| {
                org_policies::table
                    ...
                    .filter(org_policies::atype.eq(OrgPolicyType::AutomaticUserConfirmation as i32))
                    .filter(org_policies::enabled.eq(true))
    ...
    /// Any membership, including Invited and elevated roles, forbids creating another organization.
    pub fn forbids_creating_organization(&self) -> bool {
        !self.0.is_empty()
    }

    /// Any policy membership outside `org_uuid` forbids membership in `org_uuid`.
    pub fn forbids_membership_outside(&self, org_uuid: &OrganizationId) -> bool {
        self.0.iter().any(|m| &m.org_uuid != org_uuid)
    }

    /// Accepted, Confirmed, or Revoked members may neither grant nor accept emergency access; Invited is exempt.
...
mod tests {
    ...
    fn membership(org_uuid: &str, atype: MembershipType, status: MembershipStatus) -> Membership {
        let mut member = Membership::new("user".to_owned().into(), org_uuid.to_owned().into(), None);
        member.atype = atype as i32;
        member.status = status as i32;
        member
    }

    /// Every role and status is covered; only emergency access exempts Invited.
    #[test]
    fn auto_confirm_requirement() {
        ...
        // Revoked counts because restore has no second accept step; Invited alone does not restrict emergency access.
