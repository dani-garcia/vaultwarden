...
impl OrgPolicy {
    ...
    /// Combines the `Send controls` policies of every organization the user is a plain member of. Only
    /// the restrictions enforced in `sends.rs` are merged, each from the first organization that sets
    /// it like upstreams `SendControlsPolicyRequirementFactory`.
...
mod tests {
    ...
    fn send_controls(data: &str) -> SendControlsPolicyData {
        OrgPolicy::new(OrganizationId::from(String::new()), OrgPolicyType::SendControls, true, data.to_owned())
            .send_controls_data()
    }

    #[test]
    fn send_controls_data_parses_client_payloads_and_pascal_case_aliases() {
        let data = send_controls(r#"{"disableSend":true,"disableHideEmail":true,"whoCanAccess":1,"deletionHours":2}"#);
        assert!(data.disable_send && data.disable_hide_email && data.deletion_hours == Some(2));
        assert!(data.required_access_type() == Some(SendWhoCanAccessType::PasswordProtected));
        let data = send_controls(r#"{"DisableSend":true,"allowedSendTypes":[1]}"#);
        assert!(data.disable_send && !data.disable_hide_email && data.allowed_send_types == Some(vec![1]));
    }

    #[test]
    fn a_policy_without_readable_data_restricts_nothing() {
        let unrestricted = serde_json::to_value(SendControlsPolicyData::default()).unwrap();
        for unreadable in ["null", "", r#"{"whoCanAccess":"1"}"#] {
            assert_eq!(serde_json::to_value(send_controls(unreadable)).unwrap(), unrestricted, "{unreadable:?}");
        }
    }
}
