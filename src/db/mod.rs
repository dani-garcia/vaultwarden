...
const CUSTOM_ROLE_PERMISSIONS_MIGRATION: &str = "20260922120000";
...
/// `migrations/mysql/2026-09-22-120000_add_custom_role_permissions/up.sql`. Idempotent for the same
...
/// Generates the schema preflight for one database backend.
///
/// The three backends ask the same eleven questions in the same order and act on the answer with the
/// same `CustomRolePreflightDecision` match; only *how* a question is asked differs, because each
/// backend exposes its catalog differently. Those statements are the macro's parameters, so the shared
/// part exists once and a backend contributes its dialect and nothing else.
///
/// Each expanding module also provides `fn resume_interrupted(&mut Connection, CustomRoleMigrationFacts)
/// -> Result<(), Error>`. `ResumeInterruptedMigration` can only be decided when the backend declares
/// `INTERRUPTIBLE_SCHEMA_CHANGES = true` (see `custom_role_preflight_decision`), so for the other two it
/// is unreachable -- they still state their refusal explicitly rather than rely on that from a distance.
macro_rules! generate_custom_role_preflight {
    (
        connection: $conn:ty,
        table_exists: $table_exists:literal,
        column_exists: $column_exists:literal,
        columns_present: $columns_present:literal,
        columns_not_null: $columns_not_null:literal,
        column_count: $column_count:literal,
        record_migration: $record_migration:literal,
    ) => {
        ...
        fn count(connection: &mut $conn, query: impl Into<String>) -> Result<i64, diesel::result::Error> {
            diesel::sql_query(query).get_result::<Count>(connection).map(|row| row.count)
        }

        fn table_exists(connection: &mut $conn, table: &str) -> Result<bool, diesel::result::Error> {
            count(connection, format!($table_exists, table = table)).map(|value| value != 0)
        }

        /// Idempotent ledger insert, so a repeated or racing startup is a no-op rather than a
        /// duplicate-key failure.
        fn record_migration(connection: &mut $conn) -> Result<(), diesel::result::Error> {
            diesel::sql_query(format!($record_migration, version = super::CUSTOM_ROLE_PERMISSIONS_MIGRATION))
                .execute(connection)
                .map(|_| ())
        }
        ...
        fn preflight(connection: &mut $conn) -> Result<(), super::Error> {
            ...
            let access_all_column_exists = count(connection, format!($column_exists, column = "access_all"))? != 0;

            let permission_columns_present = count(
                connection,
                format!($columns_present, columns = super::sql_name_list(&super::CUSTOM_ROLE_PERMISSION_COLUMNS)),
            )?;
            let permission_columns_not_null = count(
                connection,
                format!($columns_not_null, columns = super::sql_name_list(&super::CUSTOM_ROLE_PERMISSION_COLUMNS)),
            )?;
            let membership_column_count = count(connection, $column_count)?;
            let expected_membership_columns_present = count(
                connection,
                format!($columns_present, columns = super::sql_name_list(&super::EXPECTED_MEMBERSHIP_COLUMNS)),
            )?;
            ...
            match decision {
                ...
                super::CustomRolePreflightDecision::ResumeInterruptedMigration => resume_interrupted(connection, facts),
                ...
                | super::CustomRolePreflightDecision::MaterializeLegacyUserAccessAll => {
                    ...
                    // evaluated before entering the resolution transaction. A valid interruption (only
                    // possible on a backend with interruptible schema changes) is resumed afterwards.
                    let followup = super::custom_role_decision_after_legacy_resolution(
                        facts,
                        policy,
                        INTERRUPTIBLE_SCHEMA_CHANGES,
                    );
                    ...
                    match followup {
                        super::CustomRolePreflightDecision::ResumeInterruptedMigration => {
                            resume_interrupted(connection, facts)
                        }
                        _ => Ok(()),
                    }
                }
...
mod sqlite_migrations {
    ...
    generate_custom_role_preflight! {
        connection: diesel::sqlite::SqliteConnection,
        table_exists: "SELECT COUNT(*) AS count FROM sqlite_master \
                       WHERE type = 'table' AND name = '{table}'",
        column_exists: "SELECT COUNT(*) AS count FROM pragma_table_xinfo('users_organizations') \
                        WHERE name = '{column}'",
        columns_present: "SELECT COUNT(*) AS count FROM pragma_table_xinfo('users_organizations') \
                          WHERE name IN ({columns})",
        columns_not_null: "SELECT COUNT(*) AS count FROM pragma_table_xinfo('users_organizations') \
                           WHERE name IN ({columns}) AND \"notnull\" = 1",
        column_count: "SELECT COUNT(*) AS count FROM pragma_table_xinfo('users_organizations')",
        record_migration: "INSERT OR IGNORE INTO __diesel_schema_migrations (version, run_on) \
                           VALUES ('{version}', CURRENT_TIMESTAMP)",
    }

    /// SQLite runs the whole migration inside one transaction, so it cannot stop half-way and
    /// `custom_role_preflight_decision` never resumes for it. Fail closed rather than rely on that
    /// from a distance.
    fn resume_interrupted(
        _connection: &mut diesel::sqlite::SqliteConnection,
        facts: super::CustomRoleMigrationFacts,
    ) -> Result<(), super::Error> {
        Err(super::custom_role_preflight_error(
...
mod mysql_migrations {
    ...
    // This is the backend that produces both repairable states: MySQL and MariaDB commit every
    // ALTER TABLE on their own, so a process killed part-way through leaves a database that looks
    // pending but is not.
    generate_custom_role_preflight! {
        connection: diesel::mysql::MysqlConnection,
        table_exists: "SELECT COUNT(*) AS count FROM information_schema.tables \
                       WHERE table_schema = DATABASE() AND table_name = '{table}'",
        column_exists: "SELECT COUNT(*) AS count FROM information_schema.columns \
                        WHERE table_schema = DATABASE() \
                          AND table_name = 'users_organizations' \
                          AND column_name = '{column}'",
        columns_present: "SELECT COUNT(*) AS count FROM information_schema.columns \
                          WHERE table_schema = DATABASE() \
                            AND table_name = 'users_organizations' \
                            AND column_name IN ({columns})",
        columns_not_null: "SELECT COUNT(*) AS count FROM information_schema.columns \
                           WHERE table_schema = DATABASE() \
                             AND table_name = 'users_organizations' \
                             AND column_name IN ({columns}) \
                             AND is_nullable = 'NO'",
        column_count: "SELECT COUNT(*) AS count FROM information_schema.columns \
                       WHERE table_schema = DATABASE() AND table_name = 'users_organizations'",
        record_migration: "INSERT IGNORE INTO __diesel_schema_migrations (version, run_on) \
                           VALUES ('{version}', CURRENT_TIMESTAMP)",
    }

    /// MySQL and MariaDB commit every `ALTER TABLE` on its own, so a killed process can leave the
    /// migration half-applied. `custom_role_migration_is_resumable` has already established that the
    /// remaining work is exactly the two statements below.
    fn resume_interrupted(
        connection: &mut diesel::mysql::MysqlConnection,
        _facts: super::CustomRoleMigrationFacts,
    ) -> Result<(), super::Error> {
        resume_migration(connection)
    }
...
mod postgresql_migrations {
    ...
    // Columns are resolved through the same `to_regclass` lookup as the table check, so a
    // `search_path` split cannot make the schema and the column check describe two different tables.
    // PostgreSQL has transactional DDL and never produces the "completed but unrecorded" state itself --
    // the repair is here so a database restored or copied from a MySQL-side incident is handled
    // identically on every backend.
    generate_custom_role_preflight! {
        connection: diesel::pg::PgConnection,
        table_exists: "SELECT COUNT(*) AS count FROM pg_class WHERE oid = to_regclass('{table}')",
        column_exists: "SELECT COUNT(*) AS count FROM pg_attribute \
                        WHERE attrelid = to_regclass('users_organizations') \
                          AND attnum > 0 \
                          AND NOT attisdropped \
                          AND attname = '{column}'",
        columns_present: "SELECT COUNT(*) AS count FROM pg_attribute \
                          WHERE attrelid = to_regclass('users_organizations') \
                            AND attnum > 0 AND NOT attisdropped \
                            AND attname IN ({columns})",
        columns_not_null: "SELECT COUNT(*) AS count FROM pg_attribute \
                           WHERE attrelid = to_regclass('users_organizations') \
                             AND attnum > 0 AND NOT attisdropped AND attnotnull \
                             AND attname IN ({columns})",
        column_count: "SELECT COUNT(*) AS count FROM pg_attribute \
                       WHERE attrelid = to_regclass('users_organizations') \
                         AND attnum > 0 AND NOT attisdropped",
        record_migration: "INSERT INTO __diesel_schema_migrations (version, run_on) \
                           VALUES ('{version}', CURRENT_TIMESTAMP) ON CONFLICT (version) DO NOTHING",
    }

    /// Diesel runs each PostgreSQL migration inside a transaction, so it cannot stop half-way and
    /// `custom_role_preflight_decision` never resumes for it. Fail closed rather than rely on that
    /// from a distance.
    fn resume_interrupted(
        _connection: &mut diesel::pg::PgConnection,
        facts: super::CustomRoleMigrationFacts,
    ) -> Result<(), super::Error> {
        Err(super::custom_role_preflight_error(
            super::CustomRolePreflightDecision::RefuseAmbiguousPartialMigration,
...
mod custom_role_migration_sql_tests {
    ...
    const ADD_CUSTOM_ROLE_PERMISSIONS: &str =
        include_str!("../../migrations/sqlite/2026-09-22-120000_add_custom_role_permissions/up.sql");
    ...
    fn legacy_manager_conversion() {
        ...
        // The conversion rebuilds the table, so a forgotten column would silently drop data. It also has
        // to be exactly the table the preflight recognizes as finished.
        assert_eq!(
            rows(&mut connection, "SELECT name AS value FROM pragma_table_xinfo('users_organizations')"),
            super::EXPECTED_MEMBERSHIP_COLUMNS
        );
