...
/// `existing` is the Send being updated: the client may leave the password out to keep the existing
/// one, and the deletion date is measured against the date the Send was originally created on.
///
...
async fn enforce_send_controls_policy(
    data: &SendData,
    existing: Option<&Send>,
    headers: &Headers,
    conn: &DbConn,
) -> EmptyResult {
    let has_password = data.password.is_some() || existing.is_some_and(|s| s.password_hash.is_some());
    let creation_date = existing.map_or_else(Utc::now, |s| s.creation_date.and_utc());
...
async fn post_send(data: Json<SendData>, headers: Headers, conn: DbConn, nt: Notify<'_>) -> JsonResult {
    ...
    enforce_send_controls_policy(&data, None, &headers, &conn).await?;
...
async fn post_send_file(data: Form<UploadData<'_>>, headers: Headers, conn: DbConn, nt: Notify<'_>) -> JsonResult {
    ...
    enforce_send_controls_policy(&model, None, &headers, &conn).await?;
...
async fn put_send(send_id: SendId, data: Json<SendData>, headers: Headers, conn: DbConn, nt: Notify<'_>) -> JsonResult {
    ...
    enforce_send_controls_policy(&data, Some(&send), &headers, &conn).await?;
