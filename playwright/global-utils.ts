...
async function asyncCallback(f){
    return new Promise(resolve => {
        f((...args) => { resolve(args); });
    });
}

function asyncCallbacks(f){
...
async function* emailGenerator(maildev) {
    var next = asyncCallback((callback) => {
        maildev.on("new", callback);
    });

    do {
        const [email] = await next;
        maildev.removeAllListeners("new");

        next = asyncCallback((callback) => {
            maildev.on("new", callback);
        });
...
export { asyncCallback, emailGenerator, loadEnv,  waitFor, startComposeService, stopComposeService, startVaultwarden, stopVaultwarden, restartVaultwarden };
